...
import gov.nysenate.sage.connectors.DMVConnect;
...
public class ApiServlet extends HttpServlet {
	...
	public static final String API_FORMATS = "(xml|json|kml|csv)";
    ...
    */
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) 
		...
		try {
			...
			if(format == null || !format.matches(API_FORMATS) || (!command.equals("geocode") && format.equals("csv"))) {
			...
			if(command.equals("districts")) {
				if(type.equals("addr")){
					...
					out.write(new DistrictServices().getDistrictsFromAddress(
							GeoCode.getExtendedAddress(addr2, city, state, zip4, zip5), format, service));
				...
				if(type.equals("extended")){
					out.write(GeoCode.getApiGeocodeResponse(number, addr2, city, state, zip4, zip5, format, service));
				}
				...
				if(inputType.equals("addr")){
					...
					new DistrictServices().getPolyFromAddress(
							GeoCode.getExtendedAddress(addr2, city, state, zip4, zip5), format, service, type, out);
