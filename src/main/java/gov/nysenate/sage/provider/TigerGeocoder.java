...
import gov.nysenate.sage.dao.TigerGeocoderDao;
...
public class TigerGeocoder implements GeocodeService
{
    ...
    private GeocodeQuality resolveGeocodeQuality(Address inputAddress, GeocodedStreetAddress gsa)
    {
        ...
        /** A building number usually means its a house quality match. */
        if (sa.getBldgNum() != 0 && rawQuality <= 40) { return GeocodeQuality.HOUSE; }
        ...
        if (!inputAddress.getZip5().isEmpty() && inputAddress.getZip5() == sa.getPostal()) {
