...
public class BatchDistrictResponse extends BatchResponse<DistrictResponse> {
    private BatchDistrictResponse(List<Tuple<DistrictResult, GeocodedAddress>> results) {
        super(results, result -> new DistrictResponse(result.first(), result.second(), false));
    }

    public static BatchDistrictResponse of(List<DistrictResult> districtResults, List<GeocodedAddress> geoAddrs) {
        var tupleList = new ArrayList<Tuple<DistrictResult, GeocodedAddress>>();
