...
SET muni_type = CASE
                    WHEN county_legislature IN (7, 8)
                        OR (county_legislature = 9 AND code = 8)
                        THEN 'Town'
                    ELSE 'City'
    END
...
WITH unique_names AS (
    SELECT county, base_name, min(muni_type) AS muni_type
    FROM districts.town_city
    GROUP BY county, base_name
    HAVING count(*) = 1
)
UPDATE districts.election e
...
WHERE e.muni_type IS NULL
  ...
  AND replace(e.tc_name, ' ', '') ILIKE replace(u.base_name, ' ', '');
